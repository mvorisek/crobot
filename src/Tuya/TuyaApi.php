...
class TuyaApi
{
    ...
    /**
     * @return list<int> https://developer.tuya.com/en/docs/cloud/6c61ea86c6?id=Kcp2kvgpl7bqr
     */
    public function querySpaceList(): array
    {
        $response = $this->sendRequest('get', '/v2.0/cloud/space/child?only_sub=true&page_size=1000');
        assert(count($response['data']) > 0 && count($response['data']) < 1000);

        return $response['data'];
    }

    /**
     * @return array<string, array<string, scalar>> https://developer.tuya.com/en/docs/cloud/fc0bc9f9b9?id=Kcspxd1vuxqub
     */
    public function queryDeviceList(): array
    {
        $spaceIds = $this->querySpaceList();

        $res = [];
        foreach (array_chunk($spaceIds, 5) as $spaceIdsChunk) {
            $pageSize = 20;
            $lastDeviceId = null;
            do {
                $response = $this->sendRequest('get', '/v2.0/cloud/thing/space/device?space_ids=' . implode(',', $spaceIdsChunk) . '&is_recursion=true&page_size=' . $pageSize . ($lastDeviceId !== null ? '&last_id=' . rawurlencode($lastDeviceId) : ''));
                foreach ($response as $v) {
                    $lastDeviceId = $v['id'];
                    assert(!isset($res[$lastDeviceId]));
